...
namespace B20_Ex02_Itay_311446876_Alex_322038399
{
    internal class MemoryGameLogic
    {
        ...
        internal Tuple<int, int, int> ComputerSecondChoice(Tuple<int, int> i_Item)
        {
            ...
            if (m_AIMemory.ContainsValue(valueToCheck)) // in case The computer already seen the cell that matches the first choice
            {
                // scan for valueToCheck in the AIMemory Dict -- promised to find it in this step
                foreach (var itemOfDict in m_AIMemory)
                {
                    if (valueToCheck == itemOfDict.Value) // when we find the cell we were looking for at the stoarage of the computer
                    {
            ...
            else  // in case the computer didnt see so far the the cell that matches the first choice
            {
                m_AIMemory.Add(i_Item,valueToCheck);
                int generatedNum = m_Random.Next(m_UnExposeCells.Count()); // the computer must make a randomly choice at this stage
                Tuple<int, int> keyOfFirst = m_UnExposeCells.ElementAt(generatedNum).Key;
                this.ExposeCell(keyOfFirst.Item1, keyOfFirst.Item2);
                if (CellsAreEquals(i_Item.Item1, i_Item.Item2, keyOfFirst.Item1, keyOfFirst.Item2)) // in case the computer guessed right
                {
                    scoreForMatch++;
                    returnTuple = new Tuple<int, int, int>(0, 0, scoreForMatch);
                }
                else // in case the computer did not guessed right
                {
